...
def Convection_2D(u_to,dx,dy,dt,C):
    ...
    for i in range (0,int(t/dt)):
        ...
        for i in range (0,len(u_to[:,0])): #define the y each iteration
            ...
            for j in range (0,len(u_to[0])): #define the x each iteration
                ...
                u_new[i,j] = (u_to[i,j] 
                              - C*(dt/dy)*(u_to[i,j] - u_to[i-1,j]) 
                              - C*(dt/dx)*(u_to[i,j] - u_to[i,j-1]))
...
#call the U
U_xyt = Convection_2D(u_to,dx,dy,dt,C)

#Visualize
fig, ax = plt.subplots(figsize=(8,7))

# Inisialisasi heatmap awal
cax = ax.imshow(U_xyt[0], cmap='viridis', origin='lower', extent=[0, 2, 0, 2], vmin=np.min(U_xyt), vmax=np.max(U_xyt))
