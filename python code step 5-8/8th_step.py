...
sigma = 0.0001 #stability criterion
t = 0.625   #set the t limit
nu = 0.0001   #set the diffussion's coeff value
dt = sigma*dx**2/nu  #set the timestep
...
#set the boundary condition and initial condition, at u(x,y,0)
    ...
    for i in range (0,int(t/dt)):
        ...
        for i in range (1,len(u_to[:,0])-1): #define the y each iteration
            ...
            for j in range (1,len(u_to[0])-1): #define the x each iteration
                ...
                u_new[i,j] = (u_to[i,j]
                               - dt*(u_to[i,j]*(u_to[i,j] - u_to[i,j-1])/dx 
                                     + v_to[i,j]*(u_to[i,j] - u_to[i-1,j])/dy) 
                              + nu*dt*((u_to[i,j+1] - 2*u_to[i,j] + u_to[i,j-1])/dx**2 
                                       - (u_to[i+1,j] - 2*u_to[i,j] + u_to[i-1,j])/dy**2))

                v_new[i,j] = (v_to[i,j] 
                              - dt*(u_to[i,j]*(v_to[i,j] - v_to[i,j-1])/dx
                                     + v_to[i,j]*(v_to[i,j] - v_to[i-1,j])/dy) 
                              + nu*dt*((v_to[i,j+1] - 2*v_to[i,j] + v_to[i,j-1])/dx**2 
...
cax = ax.imshow(U_xyt[0], cmap='seismic', origin='lower', extent=[0, 2, 0, 2], vmin=np.min(U_xyt), vmax=np.max(U_xyt))
fig.colorbar(cax, ax=ax)
ax.set_title('2D Burger Equation: $u(x, y, t)$')
ax.set_xlabel('$x$')
ax.set_ylabel('$y$')
...
bx.set_title('2D Burger Equation $v(x, y, t)$')
bx.set_xlabel('$x$')
bx.set_ylabel('$y$')

def animate(n):
    dax.set_data(V_xyt[n])
    bx.set_title(f'2D Burger Equation $v(x, y, t)$ at t = {n}')
    return dax,

ani = animation.FuncAnimation(fig, animate, frames=len(V_xyt), interval=30, blit=False)
#ani.save('savedata/burger_2d_animation V.mp4', writer='ffmpeg', fps=30)
