...
sigma = 0.05 #stability criterion
t = 0.625   #set the t limit
nu = 0.01   #set the diffussion's coeff value
...
def Diffussion_2D(u_to,dx,dy,dt,nu):
    ...
    for i in range (0,int(t/dt)):
        ...
        for i in range (1,len(u_to[:,0])-1): #define the y each iteration
            ...
            for j in range (1,len(u_to[0])-1): #define the x each iteration
                ...
                u_new[i,j] = ((nu*dt*((u_to[i,j+1] - 2*u_to[i,j] + u_to[i,j-1])/dx**2 
...
#call the U
U_xyt = Diffussion_2D(u_to,dx,dy,dt,nu)
...
def animate(n):
    ...
    ax.set_title(f'2D Diffussion $u(x, y, t)$ at t = {n}')
    return cax,
